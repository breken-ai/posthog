...
class TestEventsSchemaPropertyParity(ClickhouseTestMixin, HypothesisDjangoTestCase, BaseTest):
    ...
    def test_feature_flag_interfaces_work_across_event_schemas(self) -> None:
        ...
        native_uuid = _create_event(
            ...
            event="schema-parity",
            properties={
                "$feature_flags": {"checkout": "true", "disabled": "false", "secret": "false", "variant": "control"}
            },
        )
        ...
        native = execute_hogql_query(
            ...
            "JSONHas(properties, '$active_feature_flags', 1) "
        ...
        assert set(json.loads(native.results[0][4])) == {"checkout", "variant"}
        assert native.results[0][5] in ("checkout", "variant")
        assert native.results[0][6] == 1
