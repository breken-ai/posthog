...
**`ready_for_review` is not the merge gate.** The merge queue's run on a `trunk-merge/` branch is.
...
`turbo-discover.js` (`draft ? 'skip' : 'full'`) and `ci-frontend.yml`'s `fall_back` are the two reference implementations.
