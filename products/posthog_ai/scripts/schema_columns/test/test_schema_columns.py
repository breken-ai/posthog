...
def test_renders_session_recording_nullability() -> None:
    ...
    for column in (
        ...
        assert (
            next(line for line in rendered.splitlines() if line.startswith(f"`{column}` | ")).split(" | ")[2] == "NULL"
        )
...
def test_renders_usage_metric_contract() -> None:
    ...
    assert (
        "`group_type_index` | Integer | NOT NULL | Historical group type metadata; not used to scope metrics."
        in rendered
    )
    assert "`format` | String | NOT NULL | Display format: 'numeric' or 'currency'." in rendered
    assert "`math` | String | NOT NULL | Aggregation applied: 'count' or 'sum'." in rendered
